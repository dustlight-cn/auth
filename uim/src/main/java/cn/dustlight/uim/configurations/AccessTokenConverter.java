...
public class AccessTokenConverter extends DefaultAccessTokenConverter {
    ...
    public Map<String, ?> convertAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
        ...
        result.put("clientAuthorities", AuthorityUtils.authorityListToSet(authentication.getOAuth2Request().getAuthorities()));
        return result;
    }
    ...
    public OAuth2Authentication extractAuthentication(Map<String, ?> map) {
        ...
        if (oldRequest.getAuthorities() != null)
            authorities.addAll(oldRequest.getAuthorities());
        if (map.containsKey("clientAuthorities")) {
            String[] roles = (String[]) ((Collection) map.get("clientAuthorities")).toArray(new String[0]);
